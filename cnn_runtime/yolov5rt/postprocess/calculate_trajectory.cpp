...
int CalculateTraj::calculate_trajectory(DetectResultInfo &det_result_info)
{
	...
	for (int i = 0; i < detect_result_vector.size(); i++) {
		...
		float x_start = pedestrian_location[0];
		float y_start = pedestrian_location[1];
        float x_end = pedestrian_location[2];
        float y_end = pedestrian_location[3];
        float head_loc[4];
		memcpy(head_loc, detect_result_vector[i].head_location, sizeof(detect_result_vector[i].head_location));
...
int CalculateTraj::save_det_result(DetectResultInfo &det_result_info)
{
    int rval = 0;
	...
	for (int i = 0; i < detect_result_vector.size(); i++)
	{
		save_result << current_frame << " " \
		                   << detect_result_vector[i].track_id << " " \
						   << detect_result_vector[i].pedestrian_location[0] << " " \
						   << detect_result_vector[i].pedestrian_location[1] << " " \
						   << detect_result_vector[i].pedestrian_location[2] << " " \
						   << detect_result_vector[i].pedestrian_location[3] << " " \
						   << detect_result_vector[i].confidence << " " \
						   << -1 << " " << -1 << "\n";
	}
	save_result.close();
