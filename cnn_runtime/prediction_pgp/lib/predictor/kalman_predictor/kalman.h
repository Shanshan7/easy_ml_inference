...
namespace kalmantracking
{
	...
	public:
		...
		Point2f Predict(Point2f ball);

		//  getters for trajectory
		std::vector<cv::Point2f> getPredictedList() const
		{
			return _predicted_list;
		}

		std::vector<cv::Point2f> getCorrectedList() const
		{
			return _corrected_list;
		}

		std::vector<cv::Point2f> getGroundTruthList() const
		{
	...
	private:
		...
		std::vector<cv::Point2f> _predicted_list;
		std::vector<cv::Point2f> _corrected_list;
		std::vector<cv::Point2f> _GroundTruth_list;
		string _status;
	};
}
...
Point2f kalmantracking::kalman::Predict(Point2f ball)
{
	Point2f KBall;
	...
	try
	{
		...
		if (_initialized)
		{
			...
			_state = _k.predict();
			if (_bFullDebugOn)
				cout << "KF prediction: _state = " << endl
					 << " " << _state << endl
					 << endl;
			...
			if (ball.x != 0 || ball.y != 0)
			{
				...
				// Update
				if (_bFullDebugOn)
					cout << "KF correction: _measurement = " << endl
						 << " " << _measurement << endl
						 << endl;
