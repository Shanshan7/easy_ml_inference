...
void JoysonPredictionPost::PredictPoints(
    prediction::PredictedTrajectoryPoint &predicted_point, int i,
    float traj_point_num, float x0, float y0, float x, float y, float speed0,
    int8_t lane_id, float time0) {
  // predicted_point.x = x;
  // predicted_point.y = y;
  predicted_point.heading =
      atan2(predicted_point.x - x0, predicted_point.y - y0) / 3.14 * 180;
  predicted_point.speed =
      (sqrt((x - x0) * (x - x0) + (y - y0) * (y - y0))) / (6 / traj_point_num);
  predicted_point.acceleration =
      (predicted_point.speed - speed0) / (6 / traj_point_num);
...
void JoysonPredictionPost::PredictedTrajectorys(
    prediction::PredictedTrajectory &Trajectory, uint64_t obj_time_stamp) {
  enum MotionIntent {
  ...
  for (int i = 1; i < Trajectory.traj_point_num; i++) {
    ...
    PredictPoints(predicted_point, i, Trajectory.traj_point_num, x0, y0,
                  Trajectory.predicted_traj_points[i].x,
                  Trajectory.predicted_traj_points[i].y, speed0,
                  Trajectory.predicted_traj_points[i].lane_id, obj_time_stamp);
    Trajectory.predicted_traj_points[i] = predicted_point;
  }

  double trajectory = Trajectory.predicted_traj_points[11].heading -
                      Trajectory.predicted_traj_points[0]
                          .heading;  //最后一个的朝向与第一个点的朝向差值
 ...
 */
int JoysonPredictionPost::PredictedPostprocess(
    prediction::PredictedObject &cluster_object_trajectory) {
  time_t times;
  times = time(NULL);
...
// void JoysonPredictionPost::StructTomsg(
//     prediction::PredictedObject &objectstruct,
...
//     objectmsg.predicted_trajs[i].confidence =
//         objectstruct.predicted_trajs[i].confidence;
//     objectmsg.predicted_trajs[i].motion_intent =
//         objectstruct.predicted_trajs[i].motion_intent;
//     objectmsg.predicted_trajs[i].traj_start_time =
//         objectstruct.predicted_trajs[i].traj_start_time;
//     objectmsg.predicted_trajs[i].traj_end_time =
//         objectstruct.predicted_trajs[i].traj_end_time;
//     objectmsg.predicted_trajs[i].traj_point_num =
//         objectstruct.predicted_trajs[i].traj_point_num;
